...
import {
  pickOutProduct,
  getSaleNewOutOfStockDetails,
  getSaleToBeDeliveredById,
  bulkOutTonBag,
} from '../services/salesOutStockService';

export default {
  namespace: 'salesOutStockModel',
  state: {
  ...
  effects: {
    ...
    *saveFreeStorageEffect({ payload, callback }, { call, put, select }) {
      const { saveFreeStorageParams, storageOptions, productSpecOptions } = yield select(
        state => state.salesOutStockModel,
      );
      ...
      const result = yield call(pickOutProduct, params);
    ...
    *getFreeStorageEffect({ payload, callback }, { call, put }) {
      const result = yield call(getSaleNewOutOfStockDetails, payload);
    ...
    *getDetailEffect({ payload, callback }, { call, put, select }) {
      const { storageOptions } = yield select(state => state.salesOutStockModel);
      ...
      const result = yield call(getSaleToBeDeliveredById, payload);
      ...
      if (result && result.code === 1) {
        ...
        callback(result.object, 'ok', '');

        yield put({
          type: 'getProductSpecByProductIdEffect',
          payload: result.object.productId,
        });
      } else {
        callback('', 'error', result.message);
      }
    ...
    *saveProductEffect({ payload, callback }, { call, put, select }) {
      const result = yield call(bulkOutTonBag, payload);
      if (result && result.code === 1) {
        callback('', 'ok', '提交成功');
      } else {
        callback('', 'error', result.message);
