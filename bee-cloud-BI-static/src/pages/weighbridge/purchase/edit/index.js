...
export default class Index extends Component {
    ...
    getWeightMachineWebDeatil(str).then(res => {
      if (res && res.code === 1) {
        this.setState({
          ...
          inButtonState: res.object.inFactoryWeightIsConfirm
            ? 2
            : res.object.inFactoryWeight
            ? 1
            : 0,
          inFactoryWeightByManual: res.object.inFactoryWeightByManual || 0,
          inFactoryWeight:
            res.object.inFactoryWeight || res.object.inFactoryWeight === 0
              ? res.object.inFactoryWeight
              : null,

          outHaveConfirm: res.object.outFactoryWeightIsConfirm ? true : false,
          outButtonState: res.object.outFactoryWeightIsConfirm
            ? 2
            : res.object.outFactoryWeight
            ? 1
            : 0,
          outFactoryWeightByManual: res.object.outFactoryWeightByManual || 0,
          outFactoryWeight:
            res.object.outFactoryWeight || res.object.outFactoryWeight === 0
              ? res.object.outFactoryWeight
              : null,

          deductWeight: res.object.deductWeight || null,
          remark: res.object.remark || null,
          samplePushStatus: res.object.samplePushStatus || 0,
  ...
  handleSample = () => {
    const { machineId } = this.props.location.query;
    const { inHaveConfirm, inFactoryWeight } = this.state;

    if (!inHaveConfirm) {
      return message.warning('请先确认进厂称重');
    }
    if (!inFactoryWeight) {
      return message.warning('无进厂数量，无法通知取样');
    }

    const str = utils.queryString({ machineId, type: 1 });
    samplePush(str).then(res => {
      if (res && res.code === 1) {
        message.success(res.message);
        this.setState({
          samplePushStatus: 1,
  ...
  saveWeightAndRemark = func => {
    ...
    let haveConfirmDeductWeight = true,
    ...
    validateFields(['deductWeight', 'remark'], (err, values) => {
      if (!err) {
        if (values.deductWeight || values.deductWeight === 0) {
          ...
          const str = utils.queryString({
            machineId,
            type: 1,
            deductWeightByManual: 2,
            deductWeight: values.deductWeight,
          });
          confirmDeductWeight(str).then(res => {
            if (res && res.code === 1) {
              ...
              if (values.remark) {
                ...
                const str = utils.queryString({ machineId, type: 1, remark: values.remark });
  ...
  handlePrint = () => {
    this.saveWeightAndRemark(() => {
      const { validateFields } = this.props.form;
      const { machineId } = this.props.location.query;
      const { detail, deductWeight } = this.state;

      validateFields(
        ['deviceId', 'inFactoryWeight', 'outFactoryWeight', 'deductWeight', 'remark'],
        (err, values) => {
          if (!err) {
            let params = {
              machineId,
              ...
              netWeight: (
                detail.inFactoryWeight -
                detail.outFactoryWeight -
                (values.deductWeight || 0)
              ).toFixed(2),
            };
  ...
  handleOk = () => {
    const { validateFields } = this.props.form;
    const { machineId } = this.props.location.query;
    const { samplePushStatus } = this.state;

    validateFields(['deductWeight', 'remark'], (err, values) => {
      if (!err) {
        if (samplePushStatus === 0) {
          this.handleSample();
        }
        ...
        continueMachine({
          ...
          type: 1,
          deductWeight:
            values.deductWeight || values.deductWeight === 0 ? values.deductWeight : undefined,
    ...
    return (
      <div className={styles.container}>
        <Card
          ...
          extra={
            <Button
              onClick={this.handleSample.bind(this)}
              disabled={samplePushStatus ? true : false}
              type="primary"
              icon="sound"
            >
              通知取样
        ...
        >
          <div className={styles.formPart}>
            ...
            <Form layout="inline">
              <FormItem label="车牌号">{detail.trainNumber || '未知'}</FormItem>
              <FormItem label="司机">{detail.driver || '未知'}</FormItem>
              <FormItem label="联系方式">{detail.contact || '未知'}</FormItem>
              <FormItem label="承运方">{detail.carrierName || '未知'}</FormItem>
              <FormItem label="发货单位">{detail.deliveryCompany || '未知'}</FormItem>
              <FormItem label="产品名称">{detail.productName || '未知'}</FormItem>
            </Form>
          </div>
          <Divider />
          <div className={styles.formPart}>
            ...
            <Form>
              ...
              <FormItem label="进厂数量">
                {getFieldDecorator('inFactoryWeight', {
                  ...
                  <InputNumber
                    disabled={inButtonState !== 3}
                    precision={2}
                    style={{ width: '60%' }}
                    placeholder="请称重"
                    onChange={value => this.setState({ inFactoryWeight: value })}
                  />,
                )}
                {inButtonState === 0 ? (
                  <Button
                    onClick={this.getWeight.bind(this, 'in')}
                    type="primary"
                    style={{ marginLeft: 24 }}
                  >
                    进厂称重
                  ...
                  <Button
                    onClick={this.handleWeight.bind(this, 1)}
                    type="primary"
                    style={{ marginLeft: 24 }}
                  >
                    确认称重
                  ...
                  <Button
                    onClick={() => this.setState({ inButtonState: 3, inFactoryWeightByManual: 1 })}
                    type="primary"
                    style={{ marginLeft: 24 }}
                  >
                    手动修改
              ...
              <FormItem label="出厂数量">
                {getFieldDecorator('outFactoryWeight', {
                  ...
                  <InputNumber
                    disabled={outButtonState !== 3}
                    precision={2}
                    style={{ width: '60%' }}
                    placeholder="请称重"
                    onChange={value => this.setState({ outFactoryWeight: value })}
                  />,
                )}
                {outButtonState === 0 ? (
                  <Button
                    onClick={this.getWeight.bind(this, 'out')}
                    type="primary"
                    style={{ marginLeft: 24 }}
                  >
                    出厂称重
                  ...
                  <Button
                    onClick={this.handleWeight.bind(this, 2)}
                    type="primary"
                    style={{ marginLeft: 24 }}
                  >
                    确认称重
                  ...
                  <Button
                    onClick={() =>
                      this.setState({ outButtonState: 3, outFactoryWeightByManual: 1 })
                    }
                    type="primary"
                    style={{ marginLeft: 24 }}
                  >
              ...
              <FormItem label="扣重数量">
                {getFieldDecorator('deductWeight', {
                  ...
                  <InputNumber
                    ...
                    precision={2}
                    placeholder="请输入扣重数量"
                    onChange={value => this.setState({ deductWeight: value })}
                  />,
                )}
              </FormItem>
              <FormItem label="净重">
                {inFactoryWeight && outFactoryWeight
                  ? (inFactoryWeight - outFactoryWeight - (deductWeight || 0)).toFixed(2)
                  : '无'}
              </FormItem>
              <FormItem label="实际到厂时间">{detail.arrivalTime || '未知'}</FormItem>
              <FormItem label="备注">
          ...
          <div className={styles.footer}>
            <Button onClick={this.handleBack.bind(this)} style={{ marginRight: 24 }}>
              返回
            </Button>
            <Button
            ...
            <Popconfirm
              ...
              disabled={!inHaveConfirm || !outHaveConfirm}
            >
              <Button disabled={!inHaveConfirm || !outHaveConfirm} type="primary">
                提交
              </Button>
            </Popconfirm>
          </div>
