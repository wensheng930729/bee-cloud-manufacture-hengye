    ...
    validateFields((err, values) => {
      if (!err) {
        ...
        let peopleItem = people.filter(item => {
          return values.deliveryCompanyId === item.id
        })
        ...
        let params = {
          ...
          deliveryCompany: peopleItem.length ? peopleItem[0].name : undefined,
          productName: productItem.length ? productItem[0].name : undefined,
          inFactoryWeightByManual,
          arrivalTime
        }
        saveBuyWeightMachine(params).then(res => {
          if (res && res.code === 1) {
        ...
        <Card title="过磅车辆" bordered={false}>
          <div className={styles.formPart}>
            ...
            <Form>
              ...
              <FormItem label="发货单位">
                {
                  getFieldDecorator("deliveryCompanyId", {
                    rules: [{ required: true, message: '请选择发货单位' }]
                  })(
                    <Select style={{ width: '80%' }} placeholder="请选择发货单位">
                      {
