...
import { getSupplierListByCategory, getWeightMachineWebList } from '../../services/index';
...
export default class Index extends Component {
  ...
  componentDidMount() {
    ...
    getSupplierListByCategory().then(res => {
      if (res && res.code === 1) {
        this.setState({ supplier: res.object });
      }
    });
  ...
  render() {
    ...
    const { supplier, data, currentPage, pageSize, totalPage, totalRecords } = this.state;
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, row) => (
          <span
            onClick={() => router.push(`/weighbridge/purchase/edit?machineId=${row.machineId}`)}
      ...
      <div className={styles.container}>
        <div>
          ...
          <div className={styles.btnBox}>
            <Button
              onClick={() => router.push('/weighbridge/purchase/add')}
              type="primary"
              icon="plus"
            >
              新增称重
