...
@connect(({ sale, loading }) => ({
  listSearchParams: sale.listSearchParams,
  productsOptions: sale.productsOptions,

  dataObj: sale.dataObj,
  loading: loading.effects['sale/getListEffect'],
}))
class Index extends Component {
  ...
  handleFormChange = field => {
    const { dispatch } = this.props;
    dispatch({
      type: 'sale/cacheSearchParamsReduce',
      payload: field,
    });
  };
  ...
  handleFormAdd = () => {
    router.push('/sale/add');
  };

  // 跳转详情
  handleCheckDetail = row => {
    try {
      _czc1.push(['_trackEvent', '销售订单', '查看详情', '', '', '']);
    } catch (error) {}
    router.push(`/sale/detail?contractBusinessId=${row.contractBusinessId}`);
  };
