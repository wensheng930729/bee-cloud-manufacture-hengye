...
export default class Add extends Component {
  ...
  componentDidMount() {
    getCustomerListByCategory().then(res => {
      if (res.code === 1) {
        this.setState({ customers: res.object });
      }
    });
  ...
  handleAdd = () => {
    try {
      _czc1.push(['_trackEvent', '销售订单', '新增', '', '', '']);
    } catch (error) {}
    ...
    validateFields((err, values) => {
      if (!err) {
        ...
        newValues = {
          ...
          deliveryDate: moment(values.deliveryDate).format(formatter),
          ...proInfo[0],
        };
  ...
  render() {
    ...
    const { customers, products, locations } = this.state;
    ...
    return (
      <div className={styles.container}>
        <div className={styles.header}>
          <span className={styles.title}>新增销售合同</span>
          ...
          <Form>
            ...
            <FormItem label="客户">
              {getFieldDecorator('customerId', {
                ...
                <Select style={{ width: '75%' }} placeholder="请选择客户">
                  {customers.map(item => (
          ...
          <Form>
            <FormItem label="到达地">
              {getFieldDecorator('addressId', {
                ...
                <Select style={{ width: '75%' }} placeholder="请选择到达地">
                  {locations.map(item => (
                    <Option value={item.id} key={item.id}>
                      {item.name}
                    </Option>
            ...
            <FormItem label="销售方式">
              {getFieldDecorator('saleMode', {
                ...
                <Select style={{ width: '75%' }} placeholder="请选择销售方式">
                  <Option value={0}>自提</Option>
                  <Option value={1}>包运</Option>
                </Select>,
              )}
            </FormItem>
            <FormItem label="确认方">
