...
export default class Index extends Component {
  ...
  getData({ currentPage, pageSize }) {
    ...
    validateFields(['deviceName', 'status'], (err, values) => {
      if (!err) {
        let obj = {
          currentPage,
          pageSize,
  ...
  handleDelete = id => {
    configDeleteDeviceById(id).then(res => {
      if (res.code === 1) {
  ...
  handleOk = () => {
    ...
    const self = this
    const values = getFieldsValue(['status1', 'type', 'name'])
    const params = {
      ...values,
    ...
    const columns = [
      {
        ...
        render: (text, row) => (
          <div>
            ...
            <Popconfirm placement="topRight" title={'确认删除该设备？'} onConfirm={() => this.handleDelete(row.id)} okText="确认" cancelText="取消">
