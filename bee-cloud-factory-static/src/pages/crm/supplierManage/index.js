...
export default class Index extends Component {
  ...
  render() {
    ...
    const columns = [
      {
        ...
        dataIndex: 'category',
        key: 'category',
        render: (text, row) => text === 0 ? '核心供应商' : text === 1 ? '战略供应商' : text === 2 ? '一般供应商' : '',
        width: '17%'
      }, {
        title: '状态',
        dataIndex: 'status',
        ...
        dataIndex: 'carrier',
        key: 'carrier',
        render: (text, row) => text === 0 ? '否' : text === 1 ? '是' : '',
        width: '16%'
      }, {
        title: ' 操作',
        key: 'actions',
        render: (text, row) => (
          <div>
            <span onClick={() => router.push(`/crm/supplierManage/edit?id=${row.id}`)} style={{ color: '#1890ff', cursor: 'pointer' }}>查询详情</span>
          </div>
        ),
        width: '17%'
      }
    ...
    return (
      <div>
        <BreadCrumb extra={<Button type="primary" onClick={() => router.push(`/crm/supplierManage/edit`)} >新增</Button>} />
                  ...
                  <FormItem label="供应商名称">
                    {
                      getFieldDecorator('name')(
                        <Input style={{ width: 260 }} placeholder="按供应商名称进行查询" />
                      )
                    }
                  </FormItem>
